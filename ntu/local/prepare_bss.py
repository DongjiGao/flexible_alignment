...
def blind_source_separation(session_dict, tmp_dir, output_dir,
    ...
    for session in session_dict:
        ...
        command = "local/bss_justin_FastMNMF_20minrec_10sec_May20_dongji_debug.py "
        command += f"{location} {output_dir}/{session}.wav "
        command += f"{num_iter} {segsize} {fft_size} {num_channels}"

        print(f"Doing BSS on {session}")
        os.system(command)
...
def separate_channel(session_dict, output_dir):
    ...
    for session in session_dict:
        ...
        for channel, wav in enumerate(session_dict[session]):
            command = f"sox {location} -b 16 "
            command += f"{output_dir}/{wav}.wav remix {channel+1}"
            os.system(command)


def write_wav_scp(session_dict, output_dir):
    wav_scp = f"{output_dir}/wav.scp"
    with open(wav_scp, 'w') as ws:
        for session in session_dict:
            for wav in session_dict[session]:
...
#blind_source_separation(session_dict, tmp_dir, output_dir, log_dir)
    separate_channel(session_dict, output_dir)
    write_wav_scp(session_dict, output_dir)
